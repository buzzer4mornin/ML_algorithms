...
def main(args):
    ...
    for iteration in range(args.iterations):
        ...
        train_loss = sklearn.metrics.log_loss(train_target, 1 / (1 + np.exp(-train_data @ weights)))
        train_accuracy = sklearn.metrics.accuracy_score(train_target, train_data @ weights >= 0)
        test_loss = np.mean(-np.log((1 - test_target) + (2 * test_target - 1) / (1 + np.exp(-test_data @ weights))))
