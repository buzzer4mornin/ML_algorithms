...
def main(args):
    ...
    for instance in test_data:
        distances = np.linalg.norm(train_data - instance, ord=args.p, axis=1)
        neighbors = np.argsort(distances, kind="stable")[:args.k]
        if args.weights == "uniform":
            ...
            weights = np.exp(-distances[neighbors])
            weights /= np.sum(weights, keepdims=True)
        test_predictions.append(np.argmax(np.bincount(train_target[neighbors], weights)))
