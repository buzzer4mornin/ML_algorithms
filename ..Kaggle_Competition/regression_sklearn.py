...
parser.add_argument("--model", default="gbt", type=str, help="Model to use")
...
def main(args):
    if args.predict is None:
        ...
        if args.model in ["mean", "median"]:
            ...
            if args.model == "lr":
                ...
                model = [
                    ("MLP_ensemble", sklearn.ensemble.VotingRegressor([
                        ("MLP{}".format(i),
                         sklearn.neural_network.MLPRegressor(tol=0, learning_rate_init=0.01, max_iter=200,
                                                             hidden_layer_sizes=(300, 200, 100), activation="relu",
                                                             solver="adam")) for i in range(3)])), ]

            # ------------------------------------------ VOTING ENSEMBLE -----------------------------------------------
            # do, undo PolynomialFeatures
            # First, test each above individual model with CV + GridSearch.
            # After finding best models, then insert them below in order to get ensemble of them
            # ARGUMENT! -> weights = sequence of weights
            elif args.model == "VOTING":
                model = [
                    # ("poly", sklearn.preprocessing.PolynomialFeatures(2)),
                    ("voting_ensemble", sklearn.ensemble.VotingRegressor([
                        ("pr", sklearn.linear_model.PoissonRegressor(max_iter=1000)),
                        ("svr", sklearn.svm.SVR()), ])),
                ]
            # ------------------------------------------ STACKING ENSEMBLE ---------------------------------------------
            # do, undo PolynomialFeatures
            # First, test each above individual model with CV + GridSearch.
            # After finding best models, then insert them below in order to get ensemble of them
            # difference from voting ensemble is that this time we perform final_estimator on stacked results
            # ARGUMENT! -> cv = crossvalidation integer, default = 5
            elif args.model == "STACKING":
                model = [
                    # ("poly", sklearn.preprocessing.PolynomialFeatures(2)),
                    ("stacking_ensemble", sklearn.ensemble.StackingRegressor([
                        ("pr", sklearn.linear_model.PoissonRegressor(max_iter=1000)),
                        ("lr", sklearn.linear_model.LinearRegression()),
                    ], final_estimator=sklearn.linear_model.RidgeCV(alphas=np.arange(0.1, 10.1, 0.1)))),
                ]
            # ----------------------------------------- GRID SEARCH-----------------------------------------------------
            # params = {'n_neighbors': range(5, 9), 'weights': ['uniform', 'distance'], 'p': [1, 2, 3]}
            # knn = sklearn.linear_model.KNeighborsRegressor()
