...
def main(args):
    ...
    for iteration in range(args.iterations):
        ...
        train_pred_p    = sigmoid(train_data @ weights)
        train_pred      = train_pred_p >= 0.5
        train_accuracy  = np.sum(train_pred==train_target)/train_data.shape[0]
        train_loss      = sklearn.metrics.log_loss(train_target, train_pred_p)

        test_pred_p     = sigmoid(test_data @ weights)
        test_pred       = test_pred_p >= 0.5
        test_accuracy   = np.sum(test_pred==test_target)/test_data.shape[0]
        test_loss       = sklearn.metrics.log_loss(test_target, test_pred_p)
